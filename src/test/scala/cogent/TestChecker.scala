...
class TestChecker extends AnyFlatSpec :
    ...
    it should "flag bad C names" in {
        ...
        val children = Seq[Node](
                new Node.BasicState( StateInformation( "break", 1, Stereotype.None )),
                new Node.BasicState( StateInformation( "ok", 1, Stereotype.None )),
                new Node.BasicState( StateInformation( "fred!", 1, Stereotype.None )) ) 
        val rootNode = new Node.OrState( new StateInformation( "1fred", 0, Stereotype.None ), children )
